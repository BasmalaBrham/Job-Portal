                            ...
                            <h2>Job Type</h2>
                                @if ($jobTypes->isNotEmpty())
                                    ...
                                    <div class="form-check mb-2">
                                        <input value="{{(in_array($jobType->id,$jobTypeArray))? 'checked' : ''}}" class="form-check-input " name="job_type" type="checkbox" value="{{$jobType->id}}" id="job-type-{{$jobType->id}}">
...
@section('customJs')
    <script>
        $("#searchForm").submit(function(e){
            e.preventDefault();
            var url ='{{route("jobs")}}?';
            var keyword = $("#keyword").val(); // تم تعديل هنا
            ...
            // إذا كانت كلمة البحث تحتوي على قيمة
            if(keyword != ""){
                url +='&Keyword='+keyword; // تم تعديل هنا
            }
            // إذا كانت الموقع يحتوي على قيمة
            if(location != ""){
                url +='&location='+location;
            }
            // إذا كانت الفئة تحتوي على قيمة
            if(category != ""){
                url +='&category='+category;
            }
            // إذا كانت الخبرة تحتوي على قيمة
            if(experience != ""){
                url +='&experience='+experience;
            }
            // إذا كانت أنواع الوظائف تحتوي على قيم
